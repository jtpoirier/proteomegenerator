...
library(Biostrings)
ref <- readDNAStringSet(snakemake@REF_FASTA)
ref <- ref[[1]]
ref <- ref[2782:2826]
...
library(ggbio)
ga <- readGAlignments(snakemake@TUMOR, use.names=TRUE)
...
vcf <- autoplot(vcf) + xlim(2000, 3000)
tracks(Reads=rd, Coverage=cov, Variants=vcf, heights = c(0.2, 0.2, 0.1)) + ylab("")
```